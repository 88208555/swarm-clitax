...
远端纯运行时由调用方传递完整tasks，本地AutoCoord以.coord台账为准。使用任务卡、同级写入意图、冲突扫描、签名锁、基线握手与声明的依赖等待；挂起与重启后恢复原检查点。多个任务相撞时只冻结重叠路径，各自继续非冲突工作；双方保持原目标和执行权，不通过派单、接管或相互委派解决写入冲突。安全检查、心跳与运维由主代理或确定性服务承担，不另建监控智能体。

纯只读探测不以本地冲突扫描为前置门槛；只有已登记任务确实准备写入共享范围时才执行同级写入协调。
...
| S7 | 同级任务协商 | 已实现 | background/normal/high/urgent 四级排序、双向持久通知、局部继续与解锁后基线重取；不改变任务归属。 |
...
多个已有任务准备写文件时，先按 [autocoord.md](references/autocoord.md) 调用 `peer-coordinate`，显式选择 `background`、`normal`、`high` 或 `urgent`，并提供 `coordinationTimeoutMs`。宿主立即执行 `readyPaths`，持续查询 `peer-status`，并在安全检查点释放真实文件锁后调用 `peer-complete`。加急意图越过尚未取得锁的低级等待项；已有真实锁只在安全检查点释放，禁止强制抢锁。`peer-ready` 只通知重取基线，不能充当写权限；收到后必须创建新鲜 Aimlock 快照，再申请返回的签名文件锁。超时后宿主按唯一 spawnRequest 自动创建并绑定一个新任务窗口，派生任务禁止再次派生，不能在两个旧任务间循环切换。
